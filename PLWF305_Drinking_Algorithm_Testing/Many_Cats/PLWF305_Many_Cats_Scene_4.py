...
@File         : PLWF305_Many_Cats_Scene_1.py
@Interpreter Version: python 3.12
@Description: 场景4：A猫，B猫无饮水，RFID识别30次，A猫识别20次，B猫识别10次，无饮水，且在开始识别RFID开始到结束期间，发生水量波动，上下起伏在5ml范围；
'''
...
class MQTTProducer(object):
    ...
    def publish(self, client):
        ...
        # while True:
        self.getClient_id()
        self.weight_percent_decrement = 0.8
        ...
        for i in range(1, 2):
            ...
            for b in range(1, 3):
                ...
                self.msg2 = json.dumps(
                    {
                        ...
                        "content": [
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent + 5,
                                                1),
                                "ts": current_time + self.time_increment * 1,
                ...
                self.previous_weight_percent -= (self.previous_weight_percent - 5)
                self.previous_weight_percent += (self.previous_weight_percent + 5)
