...
@File         : PLWF305_Many_Cats_Scene_15.py
@Interpreter Version: python 3.12
@Description: 场景15：A猫，B猫共同饮水30S，A，B猫饮水15S，A猫RFID识别10次，每秒饮水0.3ml，B猫RFID识别5次，每秒饮水0.3ml，饮水结束后，水位波动上下3ml，波动15S后，A，B猫继续饮水，RFID识别A猫识别10次，B猫识别5次，水量稳定，稳定后，A，B猫继续饮水15S，A猫RFID识别10次,B猫RFID识别5次，每秒饮水0.3ml，数据上报；
'''
...
class MQTTProducer(object):
    ...
    def publish(self, client):
        ...
        # while True:
        self.getClient_id()
        self.weight_percent_decrement = 0.3
        ...
        for i in range(1, 2):
                ...
                self.msg3 = json.dumps(
                    {
                        ...
                        "content": [
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent + 3 - self.weight_percent_decrement * 1,
                                                1),
                                "ts": current_time + self.time_increment * 1,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent - 3 - self.weight_percent_decrement * 3,
                                                1),
                                "ts": current_time + self.time_increment * 3,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent + 3 - self.weight_percent_decrement * 5,
                                                1),
                                "ts": current_time + self.time_increment * 5,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent - 3 - self.weight_percent_decrement * 7,
                                                1),
                                "ts": current_time + self.time_increment * 7,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent + 3 - self.weight_percent_decrement * 9,
                                                1),
                                "ts": current_time + self.time_increment * 9,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent - 3 - self.weight_percent_decrement * 11,
                                                1),
                                "ts": current_time + self.time_increment * 11,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent + 3 - self.weight_percent_decrement * 13,
                                                1),
                                "ts": current_time + self.time_increment * 13,
                                "rfid": ""
                            },
                            ...
                            {
                                "weightPercent": round((int(self.start_weight_percent / 3000 * 100)), -1),
                                "weight": round(self.previous_weight_percent - 3 - self.weight_percent_decrement * 15,
                                                1),
                                "ts": current_time + self.time_increment * 15,
                                "rfid": ""
                            }
            ...
            for d in range(1, 2):
                ...
                result5 = client.publish(self.topic, self.msg14)
                status5 = result5[0]
                if status5 == 0:
                    logger.info(f"Send `{self.msg14}` to topic `{self.topic}`")
                else:
                    self.remarks = f'Failed to send message to topic {self.topic}'
                    logger.info(f"Failed to send message to topic {self.topic}")
                time.sleep(15)
                ...
                result2 = client.publish(self.topic, self.msg4)
                status2 = result2[0]
                if status2 == 0:
