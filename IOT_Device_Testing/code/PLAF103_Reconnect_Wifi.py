...
'''
@Author       : lucky.wang
@Date         : 2024/10/3 14:43
@File         : PLAF103_Reconnect_Wifi.py
@Interpreter Version: python 3.12
@Description: 1. 使用ppadb库控制手机的息屏;
'''
...
class ReconnectWIFI(object):
    def __init__(self, testTimes, intervalTime, restScreenTime):
    ...
    def reconnectWIFI(self, count):
        ...
        #
        # self.closeScreen()
        # time.sleep(self.intervalTime * 0.5)
        #
        # self.openScreen()
        # time.sleep(3)
        #
        ...
        # ControlScreen().screenOff()
        # time.sleep(5)
        #
        # ControlScreen().screenOn()
        # time.sleep(5)
        #
        # ControlScreen().unlockScreen()
        # time.sleep(5)
    ...
    # def changeConnectMode(self):
    #     try:
    #         os.system(f'adb -s {self.desired_capabilities["udid"]} tcpip 5555')
    #         time.sleep(60)
    #         self.allowConnect()
    #     except:
    #         self.allowConnect()
    #     try:
    #         os.system(f'adb -s {self.desired_capabilities["udid"]} connect 192.168.1.100:5555')
    #         time.sleep(60)
    #         self.allowConnect()
    #     except:
    #         self.allowConnect()
    ...
    def connectPhone(self):
        self.client = AdbClient(host='localhost', port=5037)  # 默认host和port
    ...
    def controlPhone(self):
        self.device, self.client = self.connectPhone()

        self.device.shell('input keyevent 26')  # 按下电源键
        logger.info('按下电源键息屏')
        time.sleep(self.restScreenTime)
        self.device.shell('input keyevent 26')  # 按下电源键
        logger.info('按下电源键亮屏')
        time.sleep(3)
        self.device.shell('input swipe 800 1500 800 100')
        logger.info('上滑屏幕解锁')
    ...
    def main(self):
        ...
        for count in range(1, self.testTimes + 1):
            ...
            if count == self.testTimes:
                logger.info(f'PLAF103重连WIFI专项自动化测试结束！')
...
if __name__ == '__main__':
    ...
    logger.info('开始执行PLAF103重连WIFI专项自动化测试...')
    try:
        reconnectWIFI = ReconnectWIFI(testTimes, intervalTime, restScreenTime)
        reconnectWIFI.main()
    except Exception as error:
        logger.info('An exception happened: \n' + str(error))
    # reconnectWIFI = ReconnectWIFI(testTimes, intervalTime)
    # reconnectWIFI.main()
