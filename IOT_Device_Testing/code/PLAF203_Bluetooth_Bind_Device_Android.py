...
@File         : PLAF203_Bluetooth_Bind_Device_Android.py
@Interpreter Version: python 3.12
@Description:1.跑该脚本的时候使用新的账号来绑定设备，且当前没有任何设备卡片;
@UpdateInfo   ：1. 2023/07/04  10:25 解绑设备后判断当前未处于设备首页时，重新启动app;
              2. 2023/07/04  10:39 增加蓝牙搜索设备耗时;
             3. 2023/07/04  10:57 在Desired Capabilities中添加keep_alive参数，并将其设置为True，保证Android手机维持常亮的状态（亲测不管用）;
                4. 2023/07/06  09:40 当蓝牙绑定失败后加上下拉屏幕的动作才点击retry;
'''
...
class BlueToothBindTestAF203(object):
    def __init__(self, testTimes, intervalTime):
        ...
        # self.MAC = 'C4:3C:B0:E4:ED:78'
        self.MAC = '34:7D:E4:BA:0B:6E'
    ...
    def reopenBluetooth(self):
        # 关闭蓝牙功能
        subprocess.run(['adb', '-s', f'{self.desired_capabilities["udid"]}', 'shell', 'am', 'start', '-a',
