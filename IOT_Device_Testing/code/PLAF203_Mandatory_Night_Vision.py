...
@File         : PLAF203_Mandatory_Night_Vision.py
@Interpreter Version: python 3.12
@Description: 1. 脚本执行开始时应保存夜视为关闭的状态;
'''
...
class AF203ChangeNightVision(object):
    def __init__(self, ):
        self.headers1 = {
            ...
            'content-length': '407',
            'host': 'test-api.dl-aiot.com',  # 修改
            'content-type': 'application/json;charset=utf-8',
            'token': '2ebc2e16ac5340ba8a8b6bee62335eff',  # 修改
        }
        ...
        self.desired_capabilities = {
            ...
            'platformVersion': '10',  # 系统版本号（在关于手机中查看） # 修改
            ...
            'udid': '3637513754573398',  # 在cmd窗口中敲adb devices获取 # 修改
    ...
    def screenShotOpen(self, count):
        ...
        self.driver.get_screenshot_as_file(rf'../result/{logFilename}--第{count}次夜视 OPEN.png')

    def screenShotClose(self, count):
        logFilename = time.strftime('%Y-%m-%d %H-%M-%S', time.localtime())
        self.driver.get_screenshot_as_file(rf'../result/{logFilename}--第{count}次夜视 CLOSE.png')  # 修改

    def main(self):
        logger.add(rf'../log/{self.logFilename}--黑白夜视切换专项测试.log', rotation='50MB', encoding='utf-8',
                   enqueue=True)  # 修改
        self.apphandle()
        time.sleep(10)
        count = 1
