...
'''
@Author       : lucky.wang
@Date         : 2024/10/3 11:29
@File         : PLAF203_Outflow.py
@Interpreter Version: python 3.12
@Description:1.当前APP有且仅有1个203的设备卡片;
#               : 2.该脚本主要用于复现出流失败的Bug;
#               : 3.跑脚本时注意要把测试手机屏幕亮度调到最低;
# @Knowledge points : 1.使用 driver.get_screenshot_as_png() 方法获取当前手机屏幕的截图。然后，使用 io.BytesIO() 函数将截图转换为字节流，并使用 Image.open() 函数将字节流转换为 PIL Image 对象。使用 image.getpixel() 函数获取指定坐标的 RGB 值，并将其输出到控制台;
'''
...
if __name__ == '__main__':
    ...
    logger.info('开始执行PLAF203出流专项自动化测试...')
    # try:
